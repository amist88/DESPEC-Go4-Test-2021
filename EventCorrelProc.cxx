   ...
   FatimaTAMEX_TimeMachine_dT[i]=0;
    ...
    for(int aa=0; aa<10; aa++){
     ...
     ///Germanium
         ...
         if(cInputMain->pGe_CF_T_Aligned[Germanium_TimeMachine_Det][Germanium_TimeMachineCh1]!=0 && cInputMain->pGe_CF_T_Aligned[Germanium_TimeMachine_Det][Germanium_TimeMachineCh2]!=0){       
             Germanium_TimeMachine_dT = cInputMain->pGe_CF_T_Aligned[Germanium_TimeMachine_Det][Germanium_TimeMachineCh2] - cInputMain->pGe_CF_T_Aligned[Germanium_TimeMachine_Det][Germanium_TimeMachineCh1];
...
//       hA_FRSWR_GeWR =  MakeTH1('I',"Correlations/FRS-Prompt_Ge/FRS-Ge_WR_dT","T Diff FRS WR -Germanium WR ",10000,-10000,10000,"Time[ns]", "Counts");
     ...
     // hA_FRS_GeE = MakeTH1('D', "Correlations/FRS-Prompt_Ge/Ge_EnergySum_allFRS", "Germanium Energy FRS (all) gated",6000, 0, 6000);
      ...
      for(int i=0; i<MAX_FRS_GATE; i++){
       ...
       hA_FRS_ZAoQ_GeEvsT[i]  = MakeTH2('D',Form("Correlations/FRS-Prompt_Ge/Z1vsAoQ_Ge/GeE_vs_FRSGe_dT/GeE_vs_FRSGe_dT_Z1vsAoQ_Gate%d",i),Form("T Diff FRS WR - Germanium T vs Germanium Energy Gate %d",i), 11000,-1000,10000,2000,0,2000,"FRS - Ge time (ns)", "Ge Energy (keV)");
...
for(int g=0; g<Germanium_MAX_DETS; g++){
	      ...
	      if(cInputMain->pGe_CF_T_Aligned[g][h]>0){
     hA_FRS_ZAoQ_GeEvsT_all->Fill((cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]),cInputMain->pGe_EAddback[g][h]);
	      }
	    }
}
     ...
     for(int g=0; g<Germanium_MAX_DETS; g++){
        ...
        //Not sure if we need this
          ///      if(g!=Germanium_SC41_Det&&g!=Germanium_SC41_Det_Digi&&g!=Germanium_TimeMachine_Det){
            ...
            if(cInputMain->pGe_CF_T_Aligned[g][h]>0 && cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]>0){
                dT_Ge_SCI=cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi];
                
               // cout<<"2dT_Ge_SCI " <<dT_Ge_SCI <<" cInputMain->pGe_CF_T_Aligned[g][h] " <<cInputMain->pGe_CF_T_Aligned[g][h] << "  cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi] " <<cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi] << endl;
       }
       
       
                    if (cInputMain->pGe_EAddback[g][h]>0) {
        GeE_Prm[Ge_mult_prompt] = cInputMain->pGe_EAddback[g][h];
        GeT_Prm[Ge_mult_prompt] = cInputMain->pGe_CF_T_Aligned[g][h];
...
//                  hA_FRS_ZAoQ_GeEvsT[gate]->Fill((cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]),cInputMain->pGe_EAddback[g][h]);
//       }
     
        ///Cut the prompt flash with 2D poly (accepts events in window)
        if(cGe_EdT_cut[gate]->Test((dT_Ge_SCI),cInputMain->pGe_EAddback[g][h])==true) {
           
                ///Energy vs WR dT all 
//                 hA_FRS_GeEvsT->Fill(cInputMain->pGe_EAddback[g][h],dT_frsge_prompt);

         ///SC41 - Gamma dT gate (individual PID Gates)
//             if((cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal])>fCorrel->GGe_SCI41_Low && (cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal])<fCorrel->GGe_SCI41_High){   
  
       ///Get the 'first' gamma -> Note this is only needed if you use the WR Ge - WR FRS (to get the actual gamma times)
           /// if(Ge_mult_prompt==1){
                   ///  Ge_FirstT_prompt=cInputMain->pGe_CF_T_Aligned[g][h];   
            ///Now FRS gated           
              ...
              if(cInputMain->pFRS_ZAoQ_pass[gate]==true ){
                ...
                // cout<<"2dT_Ge_SCI " <<dT_Ge_SCI << endl;
                                    }
        ...
        ///Correct the times for more than one gamma in an event        
            /*if(Ge_mult_prompt>1 && cInputMain->pGe_CF_T_Aligned[g][h]>0 ){              
                  dT_GeT_prompt = (cInputMain->pGe_CF_T_Aligned[g][h]-Ge_FirstT_prompt);
                  
                  dT_frsge_mult_prompt=(cInputMain->pGe_CF_T_Aligned[g][h] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]) + ABS(dT_GeT_prompt);
    ...
    if(cGe_EdT_cut[gate]->Test(GeT_Prm[m] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi],GeE_Prm[m])==true && cGe_EdT_cut[gate]->Test(GeT_Prm[n] - cInputMain->pGe_CF_T_Aligned[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi],GeE_Prm[n])==true) {
        ...
        if(cInputMain-> pGe_WR> 0){
         ...
         if(tag_all.size()>0)
                 ...
                 if(tag_all.at(i)==1 &&  (ts-ts_all.at(i))>fCorrel->GFRS_Ge_LongIso_TLow) {
                    ...
                    for(int g=0; g<Germanium_MAX_DETS; g++){
                          ...
                          if(cInputMain->pGe_EAddback[g][h]>0 &&  (g!=Germanium_SC41_Det&&g!=Germanium_SC41_Det_Digi&&g!=Germanium_TimeMachine_Det)) {
                             ...
                             GeT_Long[Ge_mult_long] = cInputMain->pGe_CF_T_Aligned[g][h];
           ...
           ///Note that Ge_FirstT_Long is not always necessarily the 'first' gamma (i.e. lowest time) since it loops over all detectors starting from 0
                        Ge_FirstT_long=cInputMain->pGe_CF_T_Aligned[g][h];
                ...
                ///This is for when there is more than 1 gamma in an event to get the correct time                    
                    if(Ge_mult_long>1 && cInputMain->pGe_CF_T_Aligned[g][h]>0 ){
                            dT_GeT_long =(cInputMain->pGe_CF_T_Aligned[g][h]-Ge_FirstT_long);
                ...
                for(int g=0; g<Germanium_MAX_DETS; g++){
                    for(int h=0; h<Germanium_CRYSTALS; h++){
                        if((g!=Germanium_SC41_Det&&g!=Germanium_SC41_Det_Digi&&g!=Germanium_TimeMachine_Det) && cInputMain->pGe_EAddback[g][h]>0){
                             ...
                             GeT_Prm_Long[Ge_mult_prompt] = cInputMain->pGe_CF_T_Aligned[g][h];
                            Ge_mult_prompt++;
...
//       hA_FRS_Fat_LT3_start_stop = MakeTH2('D', "Correlations/FRS-Prompt_Fatima/96Pd_Lifetime/FatdT_1415keV", "",1000,-50,50,1600,0,1600, "dT(ns)","Energy (keV)");
     ...
     hFat_Energy_GainMonitor_188Ta= MakeTH2('D',"FATIMA_VME/Stats/Fatima_GainMonitor188Ta","FATIMA Energy-Time Matrix 188Ta",1240,16600,29000,2500,0,5000,"Time (minutes)","Fat Energy (keV)");
      ...
      hA_FRS_ZAoQ_FatEvsT[i] = MakeTH2('D',Form("Correlations/FRS-Prompt_Fatima/Z1vsAoQ/FatE_vs_FRSFat_dT/Fat_EdT_ZAoQGate%d",i), Form("T Diff Fatima T - SCI41 vs Fatima Energy: Z vs AoQ Gate%d",i),9000,-1000,8000,1000, 0, 2000," Fat T- SCI41 (ns)", "Fatima Energy (keV)"); 
      
      hA_FRS_ZAoQ_FatE1vsE2[i] = MakeTH2('D',Form("Correlations/FRS-Prompt_Fatima/Z1vsAoQ/Gamma-Gamma/FatE1_vs_FatE2_Z1vsAoQ_gate%d",i), Form("Fatima Energy 1 vs Energy 2 Z vs A/Q Gate:%d",i),2000, 0, 2000, 2000,0,2000); 
      ...
      hA_FRS_Z1Z2_X2AoQ_FatEvsT[i] = MakeTH2('D',Form("Correlations/FRS-Prompt_Fatima/Z1Z2_X2AoQ/FatE_vs_FRSFat_dT/Fat_EdT_Z1Z2_X2AoQGate%d",i), Form("T Diff Fatima T - SCI41 vs Fatima Energy: Z1 vs Z2 AND X2 vs AoQ Gate%d",i),2750,-500,5000,2000, 0, 2000," Fat T- SCI41 (ns)", "Fatima Energy (keV)");
      
      hA_FRS_Z1Z2_X2AoQ_FatE1vsE2[i] = MakeTH2('D',Form("Correlations/FRS-Prompt_Fatima/Z1Z2_X2AoQ/Gamma-Gamma/FatE1_vs_FatE2_Z1Z2_X2AoQgate%d",i), Form("Fatima Energy 1 vs Energy 2 Z1 vs Z2 AND X2 vs AoQ Gate:%d",i),2000, 0, 2000, 2000,0,2000); 
      ...
      hA_FRS_Z1Z2_X4AoQ_FatE1vsE2[i] = MakeTH2('D',Form("Correlations/FRS-Prompt_Fatima/Z1Z2_X2AoQ/Gamma-Gamma/FatE1_vs_FatE2_Z1Z2_X2AoQgate%d",i), Form("Fatima Energy 1 vs Energy 2 Z1 vs Z2 AND X2 vs AoQ Gate:%d",i),2000, 0, 2000, 2000,0,2000);
 ...
 /**-----------------------------------------------------------------**/
  ...
  void EventCorrelProc::Make_Beta_Gamma_bPlast_SpillOff_Histos(){
     ...
     for(int j=0; j<bPLASTIC_CHAN_PER_DET; j++){  
      hbPlas_ToT_SpillOff[i][j] = MakeTH1('D', Form("Correlations/bPlast_Gamma/bPlastSpillOff/ToT Plas Det SpillOff. %d Ch. %d",  i,j), Form("ToT Spill Off Det. %d Ch. %d", i,j),20000, 0., 200000.);
      hbPlas_ToT_SpillOn[i][j] = MakeTH1('D', Form("Correlations/bPlast_Gamma/bPlastSpillOn/ToT Plas Det SpillOn. %d Ch. %d",  i,j), Form("ToT Spill On Det. %d Ch. %d", i,j),20000, 0., 200000.);
                }          
            }
  }
/**-----------------------------------------------------------------**/
...
//                             }
      ...
      for(int g=0; g<Germanium_MAX_DETS; g++){
                ...
                GeT_bPlast[Ge_mult_bPlast] = cInputMain->pGe_CF_T_Aligned[g][h];
                Ge_mult_bPlast++;
